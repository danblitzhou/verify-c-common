...
void initialize_ring_buffer(struct aws_ring_buffer *ring_buf,
  ...
  // assume(0 < size);
  size_t init_size;
  FUZZ_ASSUME_GT(init_size, 0);
  ring_buf->allocation = bounded_malloc(init_size);
  size_t position_head = nd_size_t();
  size_t position_tail = nd_size_t();
  // assume(position_head < size);
  position_head %= init_size;
  // assume(position_tail < size);
  position_tail %= init_size;
  ...
  ring_buf->allocation_end = ring_buf->allocation + init_size;
}
