...
int nd_aws_priority_queue_compare_fn(const void *a, const void *b) {
  if (a == NULL)
    return false;
  if (b == NULL)
    return false;
...
bool aws_priority_queue_is_bounded(struct aws_priority_queue *queue,
  ...
  bool b1 = aws_array_list_is_bounded(
      &queue->container, max_initial_item_allocation, max_item_size);
  bool b2 = aws_array_list_is_bounded(&queue->backpointers,
